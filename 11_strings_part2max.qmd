...
```{r}
mean_song <- spot_smaller|>
  mutate(sub_pop = str_detect(subgenre, "pop"))|>
  group_by(sub_pop)|>
  summarize(mean_title = mean(str_length(title)))|>
  select(sub_pop, mean_title)
mean_song
```
...
# Guess the output!
  ...
  str_view(spot_smaller$artist, "^K") #Starts with exactly a K
  
  str_view(spot_smaller$album_release_date, "01$") #ends with 01
  
  str_view(spot_smaller$title, "^.. ") # Starts with..
  
  str_view(spot_smaller$artist, "[^A-Za-z ]")#anything except "A-Za-z
```
...
```{r}
bigspotify |>
  filter(str_detect(bigspotify$track_name, "\\$", ))|>
