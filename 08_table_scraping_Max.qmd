...
```{r}
session <- bow("https://tf.tfrrs.org/lists/5020/2025_NCAA_Division_III_Outdoor_Qualifying_List?gender=m", force = TRUE)
result <- scrape(session, query=list(t="5000 Meters", per_page=100))
  filtered_results <- results[grepl("5000", results$Event, ignore.case = TRUE), ]
...
```{r}
bind_cols(mpls_data1, mpls_data2) |> #combines 2 columns together
  as_tibble() |> #Converts the resulting data frame into a tibble
  select(-`...8`) |> #Removes the column
  print(mutate(`...1` = str_extract(`...1`, "[^ ]+ [^ ]+ [^ ]+"))) |> #Extracts the first three words from the column
  pivot_longer(cols = c(`JanJa`:`DecDe`), #Converts wide-format data into long format
               names_to = "month", values_to = "weather") |>
  pivot_wider(names_from = `...1`, values_from = weather) |> #Spreads the data back into a wider format
  mutate(month = str_sub(month, 1, 3))  |> #Shortens the month column to the first three characters
  rename(avg_high = "Average high in",
         avg_low = "Average low in") #renames variables to be better to work with
