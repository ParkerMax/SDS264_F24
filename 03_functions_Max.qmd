...
The call. option makes the error message shorter or longer depending on the = value. If TRUE, it is a longer message.
...
Because x is a vector, we should use is.na(sum(x)) since if expects a TRUE FALSE value. 
...
flights |>
  ...
  summarize(mean_arrival = mean(arr_delay), se_arr = SE(arr_delay))
```
...
Initially, I believe yes because it is possible that a plane is delayed at departure but makes the time up while in the air or leaves on time, but gets placed in a holding pattern and delayed.
...
f1: calcualates difference between two times by converting them to a standard time, new name: time_diff
f2: takes the length and width in centimeters and converts to inches. New name: cm_to_in
f3: takes responses that all fall under the same category and makes a single category. Name: non_answer
...
greet(x = "2018-05-03 17:38:01 CDT")
greet(x = Sys.time())
...
```{r}
summary6_missing_rm <- function(data, var, secondvar, na.rm = TRUE) {
  if (na.rm) {
    data <- data |> 
      filter(!is.na({{var}})) }
  else {
...
mpg |> summary6_missing_rm(hwy)
mpg_tbl <- as_tibble(mpg)
gss_tbl <- as_tibble(gss_cat)
summary6_missing_rm(mpg_tbl, hwy)        
summary6_missing_rm(gss_tbl, age)        
summary6_missing_rm(gss_tbl, age, na.rm = FALSE)
summary6_missing_rm(mpg_tbl, hwy, secondvar = "drv")
summary6_missing_rm(gss_tbl, age, secondvar = "partyid")

partyid <- summary6_missing_rm(gss_tbl, age, na.rm = FALSE)
...
library(ggplot2)
library(ggrepel)
scatterfun <- function(data, center, spread) {
  ggplot(data, aes(x = .data[[center]], y = .data[[spread]])) + 
    geom_point(aes(size = n)) +
    geom_smooth(method = "lm") +
    geom_label_repel(aes(label = partyid)) +
    labs(
      title = paste(center, "vs", spread, "by party id"),
      x = center,
      y = spread)
}
print(scatterfun(party_age, center = "mean", spread = "sd"))
print(scatterfun(party_age, center = "median", spread = "IQR"))
